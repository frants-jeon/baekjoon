  ...
  for _ in range(N):
    ...
    if x == 0: # 출력해야 하는 경우
      ...
      while heap_len >= i * 2:
        ...
        if heap_len == i * 2:
          left = heap[i * 2]
          if heap[i] >= left: break # 자식보다 커서 정렬이 끝났으면 중단
          # 자식이 더 큰경우에는 자식이랑 값 교체하면 정렬 끝
          heap[i], heap[i * 2] = left, heap[i]
        else: #양쪽 자식이 다 있는 경우
          left = heap[i * 2]
          right = heap[i * 2 + 1]
          if heap[i] >= left and heap[i] >= right: break # 양쪽 자식들보다 값이 더 커서 정렬 필요 없으면 중단
          # 값이 더 큰 자식이랑 교체해야 함. 교체 후 i는 자식의 idx로 갱신.
          if left >= right:
            heap[i], heap[i * 2] = left, heap[i]
            i *= 2
          else:
      ...
      # heap은 idx 1부터 시작이기 때문에 부모가 0보다 크고 부모의 값이 더 작으면 부모의 값과 x값 교체.
